...
set.seed(18)  # my seed number is 18 (want to save and fix the number)
rnorm(5,10,3)
...
summary(sample_norm)
median(sample_norm)
IQR(sample_norm) # IQR MEANS Q3-Q1
...
par(mfrow=c(1,3))  # draw 1-by-3 figures 一行三列
hist(sample_norm)
...
hist(samp_b)
hist(samp_a)
hist(samp_b, add=T) #add sample b to the sample a
...
grp_ab  <- c(rep("a",100),rep("b",100)) #rep(1,5) means repeat 1 five times
