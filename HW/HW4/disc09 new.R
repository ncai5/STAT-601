...
ls.fit0=lm(y~1)
ls.fit=lm(y~x1+x2+x3)
anova(ls.fit0,ls.fit)
# cheking correlation betw vars
pairs(~y+x1+x2+x3);lm(x1~x2+x3)


# SS(X1|X2) = SSE(X2)-SSE(X1,X2)

anova(lm(y~x2+x1));anova(lm(y~x1+x2))
...
my.regsub(data[,1:3], data[,4], nbest = 2,method = "exhaustive")
...
#
summary(lm(y~x1+x2+x3));anova(lm(y~x1+x2+x3));anova(lm(y~x3+x1+x2))
# for example, square of t statistic for X2 in lm(y~x1+x2+x3) is the same as 
# MS(X2|X1,X3) / MSE 
